  ...
  'widget',
  {
    hover: (props, monitor, component) => {
      ...
      const hoveredIndex = component._offsets.reduce(
        (hovered, offset, i) =>
          i > hovered &&
          Math.abs(currentOffset - offset) < Math.abs(currentOffset - component._offsets[hovered])
            ? i
            : hovered,
        0,
      )
...
class Col extends Component {
  ...
  updateOffset() {
    this._offsets = _.flow(
      ...
      keys =>
        _.map(keys, k => {
          const n = findDOMNode(this.refs[k]) // eslint-disable-line
  ...
  render() {
    const { hasWidgets, widgetsIds, col, editMode, connectDropTarget, isOver } = this.props
